...
just as make-believe, has to take into account a completely different set of concerns. In this exercise we've tried to 
...
MVP, we've set up three different configurations, one per each of your usual basic environments (prod, qa, dev) based on 
Spring profiles and System variables, out of simplicity:
...
the in-memory H2 database web console. H2 was the storage solution selected for this project, bearing in
...
Once again, it was initially envisioned to expose JMX communication from the service for an appropriate client (e.g. 
Dynatrace, or other), but on a second thought it was judged as way too ambitious for an exercise like this. Let's think
about, for example, the security requirements. Also, an in-house tracing solution, akin to a database auditory table, was
projected, but this idea was also dropped for similar reasons. Not much attention has been given, either, to setting up
a custom system of logs (format, rotation, purging, etc.) since the default logging of the app (based on Java Logback)
is more than enough at this point of its development. Again, this should be of importance in an actual project, but 
we believe it falls outside the scope of the assessment. Just Spring's Actuator has been implemented, and even that 
while leaving as a pending task some of its requirements (e.g., setting up HTTPS and replacing the BASIC auth scheme).
...
documentation. The database is created and populated out of scripts (no ORM-generation here): 'schema.sql' and 'data.sql'.
...
to be offered in an unrestricted and public way as open services. If that were the case, no stringent security 
...
Actuator endpoints in the dev and qa environments (the second standing more restricted than the first).
...
unit test coverage to all the layers of the service was too much for this type of exercise, even if necessary in any
...
Regarding the development flow and code-branching strategy, the initial intent was to adopt the GitHub Flow style, given 
...
for now. No provisions for API analysis tools either (42Crunch?).
...
etc.) and a more complex clean/hexagonal architecture (ports, adapters, use cases, domain...). We finally decided to go
...
to plug in and out any integrations and, on a different note, the code base will never grow that much. The directory 
